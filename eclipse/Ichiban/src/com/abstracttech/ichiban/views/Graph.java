	...
	protected void onDraw(Canvas canvas)
	{
		...
		{					
			...
			for (int i = 0; i < data.size() - 1; i++)		//Set points to draw lines
			{
				...
				points[i * 4 + 1] = this.getHeight() * (1 - Float.parseFloat(data.toArray()[i].toString()) / top) - 2;
				points[i * 4 + 2] = this.getWidth() * (i + 1) / (float)data.size();
				points[i * 4 + 3] = this.getHeight() * (1 - Float.parseFloat(data.toArray()[i + 1].toString()) / top) - 2;
				
				pointsShadow[i * 4] = this.getWidth() * i / (float)data.size() + 5;
				pointsShadow[i * 4 + 1] = this.getHeight() * (1 - Float.parseFloat(data.toArray()[i].toString()) / top) + 5;
				pointsShadow[i * 4 + 2] = this.getWidth() * (i + 1) / (float)data.size() + 5;
				pointsShadow[i * 4 + 3] = this.getHeight() * (1 - Float.parseFloat(data.toArray()[i+1].toString()) / top) + 5;
			}
