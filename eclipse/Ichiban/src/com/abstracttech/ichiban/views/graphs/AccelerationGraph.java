...
public class AccelerationGraph extends ImageView {
	...
	protected void onDraw(Canvas canvas)
	{
		...
		canvas.drawText("a(t)", this.getWidth() - 20, 20, p);
		...
		{					
			...
			for (int i = 0; i < data.size() - 1; i++)		//Set points to draw lines
			{
				...
				points[i * 4 + 1] = this.getHeight() * (1 - Float.parseFloat(data.toArray()[i].toString())) - 2;
				points[i * 4 + 2] = this.getWidth() * (i + 1) / (float)data.size();
				points[i * 4 + 3] = this.getHeight() * (1 - Float.parseFloat(data.toArray()[i + 1].toString())) - 2;
